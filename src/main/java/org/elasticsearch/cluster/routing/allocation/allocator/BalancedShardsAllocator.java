...
import com.kedacom.ga.MyAllocator;
...
//        final Balancer balancer = new Balancer(logger, allocation, weightFunction, threshold);
        final Balancer balancer = new Balancer(logger, allocation, weightFunction, threshold, settings);
        // patch
...
//        Balancer balancer = new Balancer(logger, allocation, weightFunction, threshold);
        Balancer balancer = new Balancer(logger, allocation, weightFunction, threshold, settings);
        // patch
    ...
    public static class Balancer {
        ...
        private int data_node_count = 1;
        public Balancer(Logger logger, RoutingAllocation allocation, WeightFunction weight, float threshold, Settings settings) {
            this.logger = logger;
            ...
            this.data_node_count = settings.getAsInt("data_node.count", 1);
        }
        ...
        private void balanceByWeights() {
        	...
        	Map<Integer, ModelNode> idx_mn = new HashMap<>();
        	Map<String, Integer> nodeid_idx = new HashMap<>();
        	List<ShardRouting> ss = new ArrayList<ShardRouting>();
            for (ModelNode node : nodes.values()) {
            	idx_mn.put(node.getNodeIndex(), node);
            	nodeid_idx.put(node.getNodeId(),node.getNodeIndex());
            	for (ModelIndex mi : node) {
            		for (ShardRouting sr : mi) {
            			ss.add(sr);
            		}
            	}
            }
            ...
            out:
            for (ShardRouting sr : ss) {
            	int num_replica = metaData.index(sr.getIndexName()).getNumberOfReplicas();
            	List<Integer> slots = MyAllocator.getShardSlots(sr.getIndexName(), sr.getId(), this.data_node_count, num_replica);

            	int current_node_idx = nodeid_idx.get(sr.currentNodeId());
            	if (sr.getType() == ShardRouting.Type.CRAWL) {
	            	for (int i = 1; i < num_replica + 1; i ++) {
	            		if (slots.get(i) == current_node_idx) {
	            			continue out;
	            		} 
	            	}
            	} else {
            		if (slots.get(0) == current_node_idx) {
            			continue out;
            		}
            	}
            	
            	
            	// this shard is not on its expected node
            	ModelNode mn = null;
            	Decision decision = null, decision_all = null;
            	Decision rebalanceDecision = null;
            	
            	List<Integer> expected_slots = new ArrayList<Integer>(data_node_count);
            	
            	if (sr.getType() != ShardRouting.Type.CRAWL) {
            		expected_slots.add(slots.get(0));
            	} else {
            		for (int ii = 1; ii < num_replica + 1; ii++) {
            			expected_slots.add(slots.get(ii));	
            		}
            	}
            	
            	ModelNode current_node = idx_mn.get(current_node_idx);
            	for (int ii : expected_slots) {
            		mn = idx_mn.get(ii);
					if (mn != null) {
						tryRelocateShard(mn, current_node, sr);
					}
            	}
            }
...
//            }
        ...
        private void allocateUnassigned() {
            ...
            do {
                for (int i = 0; i < primaryLength; i++) {
                    ...
                    // patch
                    shard.setType(ShardRouting.Type.FLY);
                    
                	IndexMetaData im = metaData.getIndices().get(shard.getIndexName());
                	Settings s = im.getSettings();
                	String allocation_type = s.get("index.allocation_type", "crawl");
                	boolean allocation_type_assigned = false;
                	
                	for (ModelNode nn : nodes.values()) {
                		ModelIndex mi = nn.getIndex(shard.getIndexName());
                		if (mi != null) {
                			for (ShardRouting sr : mi) {
                				if ((sr.getId() == shard.getId()) && (sr.getType() == ShardRouting.getAllocationType(allocation_type)) ) {
                					allocation_type_assigned = true;
                				}
                			}
                		}
                	}
                	
                	if (!allocation_type_assigned) {
                		shard.setType(ShardRouting.getAllocationType(allocation_type));
                	} else {
                		shard.setType(ShardRouting.Type.CRAWL);
                	}
                	// patch
        ...
        private AllocateUnassignedDecision decideAllocateUnassigned(final ShardRouting shard, final Set<ModelNode> throttledNodes) {
            ...
            int num_replica = metaData.index(shard.getIndexName()).getNumberOfReplicas();

            List<Integer> slots = MyAllocator.getShardSlots(shard.getIndexName(), shard.getId(), this.data_node_count, num_replica);
        	
            // we divide the slot's to three types: 
            // the ones that a shard's type(fast, slow, crawl) should locate normally
            // the ones that a shard's other type(fast, slow, crawl) should locate normally
            // all other slots
            // so we first try to locate to our normal slot, when it's not available, we try to
            // locate in 'other' slots, if we locate in the other types slots, it will stop other's type
            // to locate in their normal slot
            
        	List<Integer> slots_fast = new ArrayList<Integer>(1);
        	List<Integer> slots_slow = new ArrayList<Integer>(num_replica);
        	List<Integer> slots_other = new ArrayList<Integer>(this.data_node_count-num_replica-1);
        	
        	int ii = 0;
        	slots_fast.add(slots.get(ii));
        	for (ii = 1; ii < num_replica+1; ii++) {
        		slots_slow.add(slots.get(ii));
        	}
        	for (; ii < this.data_node_count; ii++) {
        		slots_other.add(slots.get(ii));
        	}
        	
        	List<Integer> slot_queue = new ArrayList<Integer>(this.data_node_count);
        	
        	if (shard.getType() == ShardRouting.Type.CRAWL) {
        		slot_queue.addAll(slots_slow);
            	slot_queue.addAll(slots_other);
            	slot_queue.addAll(slots_fast);
        	} else { // the only fast type
        		slot_queue.addAll(slots_fast);
            	slot_queue.addAll(slots_other);
            	slot_queue.addAll(slots_slow);
        	}
        	
            for (int iii : slot_queue) {
            	ModelNode mn = idx_mn.get(iii);
            	if (mn != null) {
            		// here we only check if there is already a shard with the same shardid
            		ModelIndex mi = mn.getIndex(shard.getIndexName());
            		if (mi != null) {
            			boolean canAlloc = true;
            			for (ShardRouting sr : mi) {
            				if (sr.getId() == shard.getId()) {
            					canAlloc = false;
            					break;
            				}
            			}
            			...
            			if (canAlloc) {
            				decision = allocation.deciders().canAllocate(shard, mn.getRoutingNode(), allocation);
		 ...
		 * with respect to the weight function. All given shards must be unassigned.
		 */
		private void allocateUnassigned_2() {
		    RoutingNodes.UnassignedShards unassigned = routingNodes.unassigned();
		    assert !nodes.isEmpty();
		    if (logger.isTraceEnabled()) {
		        logger.trace("Start allocating unassigned shards");
		 ...
		 * is of type {@link Type#NO}, then the assigned node will be null.
		 */
		private AllocateUnassignedDecision decideAllocateUnassigned_2(final ShardRouting shard, final Set<ModelNode> throttledNodes) {
		    if (shard.assignedToNode()) {
		        // we only make decisions for unassigned shards here
		        return AllocateUnassignedDecision.NOT_TAKEN;
		    }
		 ...
		 * to <tt>0.0</tt>
		 */
		private void balanceByWeights_2() {
		    final AllocationDeciders deciders = allocation.deciders();
		    final ModelNode[] modelNodes = sorter.modelNodes;
		    final float[] weights = sorter.weights;
		    for (String index : buildWeightOrderedIndices()) {
		 ...
		 * simulation model as well as on the cluster.
		 */
		private boolean tryRelocateShard(ModelNode minNode, ModelNode maxNode, ShardRouting shard) {
		    final ModelIndex index = maxNode.getIndex(shard.getIndexName());
		    Decision decision = null;
		    if (index != null) {
		        if (logger.isTraceEnabled()) {
		            logger.trace("Try relocating shard for index index [{}] from node [{}] to node [{}]", shard.getIndexName(), maxNode.getNodeId(),
		                    minNode.getNodeId());
		        }
		        ShardRouting candidate = shard;
		        final AllocationDeciders deciders = allocation.deciders();
		            if (shard.started()) {
		                // skip initializing, unassigned and relocating shards we can't relocate them anyway
		                Decision allocationDecision = deciders.canAllocate(shard, minNode.getRoutingNode(), allocation);
    ...
    static class ModelNode implements Iterable<ModelIndex> {
        ...
        ModelNode(RoutingNode routingNode) {
            ...
            try {
            	String sni = routingNode.node().getAttributes().get("data_node_index");
            	this.node_index = Integer.parseInt(sni);
            } catch (Exception e) {
            	e.printStackTrace();
            }
        }
