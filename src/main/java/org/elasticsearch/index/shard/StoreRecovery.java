...
final class StoreRecovery {
    ...
    private void internalRecoverFromStore(IndexShard indexShard) throws IndexShardRecoveryException {
        ...
        // patch
        Path sp = indexShard.shardPath().getDataPath();
        Map<String, String> attr = indexShard.indexSettings().getNodeSettings().getAsSettings("path.data_attr").getAsMap();
        for (String p : attr.keySet()) {
        	Path pp = java.nio.file.Paths.get(p);
        	if (sp.startsWith(pp)) {
        		
        		
        		String path_attr = attr.get(p);
        		
        		indexShard.routingEntry().setType(ShardRouting.getAllocationType(path_attr));
        	}
        }
        
//        indexShard.routingEntry().setType(ShardRouting.Type.CRAWL);
        // patch
