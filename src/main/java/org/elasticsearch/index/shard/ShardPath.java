...
public final class ShardPath {
	...
	public static ShardPath selectNewPathForShard(NodeEnvironment env, ShardId shardId, IndexSettings indexSettings,
		...
		if (indexSettings.hasCustomDataPath()) {
			...
			// TODO: this is a hack!! We should instead keep track of incoming
			// (relocated) shards since we know
			// how large they will be once they're done copying, instead of a
			// silly guess for such cases:

			// Very rough heuristic of how much disk space we expect the shard
			// will use over its lifetime, the max of current average
			// shard size across the cluster and 5% of the total available free
			// space on this node:
			BigInteger estShardSizeInBytes = BigInteger.valueOf(avgShardSizeInBytes)
					.max(totFreeSpace.divide(BigInteger.valueOf(20)));

			// TODO - do we need something more extensible? Yet, this does the
			// job for now...
    ...
    public static ShardPath selectNewPathForShard(NodeEnvironment env, ShardId shardId, IndexSettings indexSettings,
                                                  long avgShardSizeInBytes, Map<Path,Integer> dataPathToShardCount, ShardRouting.Type routing_type) throws IOException {
        ...
        if (indexSettings.hasCustomDataPath()) {
            ...
            Map<String, String> attr = indexSettings.getSettings().getAsSettings("path.data_attr").getAsMap();
            
            Path pp = null;
            
            for (String p : attr.keySet()) {
            	if (attr.get(p).equals("fly") && routing_type == ShardRouting.Type.FLY) {
            		pp = Paths.get(p);
            		break;
            	}
            	if (attr.get(p).equals("fast") && routing_type == ShardRouting.Type.FAST) {
            		pp = Paths.get(p);
            		break;
            	}
            	if (attr.get(p).equals("slow") && routing_type == ShardRouting.Type.SLOW) {
            		pp = Paths.get(p);
            		break;
            	}
            	if (attr.get(p).equals("crawl") && routing_type == ShardRouting.Type.CRAWL) {
            		pp = Paths.get(p);
            		break;
            	}
            }
            ...
            for (NodeEnvironment.NodePath nodePath : paths) {
				...
				if (pp == null) {
					...
					if (nodePath.path.startsWith(pp)) {
						bestPath = nodePath;
						break;
					}
				}
